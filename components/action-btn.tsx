...
// 버튼 타입별 스타일 정의
const BUTTON_STYLES = {
  add: "bg-violet-600 hover:bg-violet-700 text-white disabled:bg-slate-200 disabled:text-slate-900",
...
const ActionBtn = ({ type, active = false, onClick }: ActionBtnProps) => {
  ...
  return (
    ...
    >
      ...
      {/* 모바일 환경에서는 텍스트 숨기기*/}
      <span className="tablet:block hidden">{BUTTON_TEXTS[type]}</span>
    </button>
  );
};
